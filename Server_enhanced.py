...
def login(connectionSocket):
    ...
    # grab info from badLogin.json
    with open("bad_login.json", 'r') as f:
        badLoginDict = json.load(f)
        f.close()
    
    message = ""
    
    if userPass[0] in badLoginDict:
        if badLoginDict[userPass[0]] >= 3:
    ...
    if loggedIn:
        ...
        # Check for and update info if necessary
        if username in badLoginDict:
            badLoginDict[username] = badLoginDict[username] + 1
            print("Bad login attempts by " + username + " is now at " + str(badLoginDict[username]))
            # write info back to file.

    connectionSocket.send(message)  # Send appropriate message if logged in or not

    with open("bad_login.json", "w") as outfile:
        json.dump(badLoginDict, outfile)
        outfile.close()
